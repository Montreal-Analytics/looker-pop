...
### CURRENT TIMESTAMP {
  ...
  dimension_group: selected_reference_date_default_today_advanced {
    ...
    sql:
      case
        when {% parameter parameters.select_reference_date_advanced %} is null or date(${parameters.current_timestamp_advanced_date}) <= date({% parameter parameters.select_reference_date_advanced %})
          then date(${parameters.current_timestamp_advanced_date})
