    ...
    sql:
      {% if parameters.select_timeframe_advanced._parameter_value == "ytd" %}
        case
          ...
          when ${order_items.created_date} BETWEEN date_trunc('year', date_add('year', -1,${parameters.selected_reference_date_default_today_advanced_raw})) and date_add('year', -1, ${parameters.selected_reference_date_default_today_advanced_date})
            then ${selected_dynamic_timeframe_advanced}
          else null
        end
      {% else %}
        {% if parameters.select_comparison._parameter_value == "year" %}
          case
            when date_trunc('{% parameter parameters.select_timeframe_advanced %}', ${order_items.created_raw}) = date_trunc('{% parameter parameters.select_timeframe_advanced %}', ${parameters.selected_reference_date_default_today_advanced_raw})
              then ${selected_dynamic_timeframe_advanced}
            when date_trunc('{% parameter parameters.select_timeframe_advanced %}', ${order_items.created_raw}) = date_trunc('{% parameter parameters.select_timeframe_advanced %}', date_add('year', -1, ${parameters.selected_reference_date_default_today_advanced_raw}))
              then ${selected_dynamic_timeframe_advanced}
          end
        {% elsif parameters.select_comparison._parameter_value == "period" %}
          case
            when date_trunc('{% parameter parameters.select_timeframe_advanced %}', ${order_items.created_raw}) = date_trunc('{% parameter parameters.select_timeframe_advanced %}', ${parameters.selected_reference_date_default_today_advanced_raw})
              then ${selected_dynamic_timeframe_advanced}
            when date_trunc('{% parameter parameters.select_timeframe_advanced %}', ${corder_items.created_raw}) = date_trunc('{% parameter parameters.select_timeframe_advanced %}', date_add('{% parameter parameters.select_timeframe_advanced %}', -1, ${parameters.selected_reference_date_default_today_advanced_raw}))
              then ${selected_dynamic_timeframe_advanced}
          end
        {% endif %}
      {% endif %}
      ...
      {% if parameters.select_timeframe_advanced._parameter_value == "ytd" %}
        case
          when ${order_items.created_raw} BETWEEN date_trunc('year', ${parameters.selected_reference_date_default_today_advanced_raw}) and ${parameters.selected_reference_date_default_today_advanced_raw}
            then 'reference'
          when ${order_items.created_raw} BETWEEN date_trunc('year', date_add(year, -1, ${parameters.selected_reference_date_default_today_advanced_raw})) and date_add(year, -1, ${parameters.selected_reference_date_default_today_advanced_date})
            then 'comparison'
          else null
        end
      {% else %}
        ...
        {% elsif parameters.select_comparison._parameter_value == "period" %}
          case
            when date_trunc('{% parameter parameters.select_timeframe_advanced %}', ${order_items.created_raw}) = date_trunc('{% parameter parameters.select_timeframe_advanced %}', ${parameters.selected_reference_date_default_today_advanced_raw})
              then 'reference'
            when date_trunc('{% parameter parameters.select_timeframe_advanced %}', ${order_items.created_raw}) = date_trunc('{% parameter parameters.select_timeframe_advanced %}', date_add('{% parameter parameters.select_timeframe_advanced %}', -1, ${parameters.selected_reference_date_default_today_advanced_raw}))
              then 'comparison'
          end
        {% endif %}
      {% endif %}
